/* =====================================================================
   תאוריה מדברת · שכבת ההקראה שבאפליקציה
...
const CASES = [
  ...
  ['חשוב על שני מספרי ברירת מחדל.',                    'בְּרֵירַת']
];
...
/* ---- מספרי תמרור ---- */
{
  ...
  /* רשימת מספרי תמרור היא אפשרות אחת בשאלה, ולכן היא מזהה ולא
     כמות — אחרת "136, 135" נקרא "מאה שלושים ושש" ואי אפשר להבדיל
     בין שתי האפשרויות באוזן. */
  const d = speechMap('136, 135').spoken;
  ok('רשימת מספרי תמרור נקראת ספרה-ספרה',
     /אחת\s+שלוש\s+שש,\s*אחת\s+שלוש\s+חמש/.test(d), d);

  /* קוד עם אות: הספרות ספרה-ספרה גם כשהן שתיים בלבד, כי כאן הן
     שם ולא כמות. */
  const e = speechMap('ס-31').spoken;
  ok('קוד תמרור עם אות נקרא ספרה-ספרה', /ס-שלוש\s+אחת/.test(e), e);

  /* האות שצמודה לספרה חייבת רווח, אחרת נוצרת "שבעפ" */
  const f = speechMap('127פ').spoken;
  ok('אות צמודה לספרה מופרדת ברווח', /שבע\s+פ/.test(f), f);

  /* ולא מרחיבים את זה על מהירות דו-ספרתית שעומדת לבדה */
  const g = speechMap('50').spoken;
  ok('מספר דו-ספרתי לבדו נשאר כמות', g === '50', g);
}
