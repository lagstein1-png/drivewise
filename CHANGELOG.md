...
экрану״ המודל החזיר ״Переходим к следующему вопросу.״ **בלי** קריאת
פונקציה, והשרת החזיר 200. הלומד היה רואה הודעה על מעבר שלא קרה —
בדיוק הכשל שמנוע ברק נבנה לסגור, והפעם דרך תשובה מוצלחת.

`barak-core.js`: כשהשרת ענה בלי פעולה והטקסט בכל זאת מכריז עליה,
הפעולה מתבצעת מקומית דרך המתאם — ואם היא נכשלת, הטקסט מוחלף.
**שני תנאים יחד:** הלומד ביקש במפורש, וגם המודל הכריז. ״מה הצעד
הבא?״ מפעיל את הראשון לבדו ואינו בקשה; תנאי אחד לבדו היה מדלג
לשאלה הבאה באמצע הסבר.

הקובץ כאן הוא עותק מקומי של זה שבריפו הראשי, והתיקון זהה מילה
במילה. 17 מתוך 17 חבילות עברו.

## v100 - ״עזרה מהמורה״: ברק נכנס לאפליקציה, ויודע מה על המסך

מנוע ברק (BARAK-CORE, 16.9.2026). כפתור ״ברק — עזרה מהמורה״ במסך
הרצף ובתרגול (לא בבחינה). הפאנל, הפנים והמוח המקומי מגיעים
מ-`/tutor/` של הריפו הראשי — אותו מקור, כמו `/legal/`; `barak-core.js`
הוא עותק מקומי כאן. מה שנשלח לשרת (רק כשיש רשת): השאלה, ארבע
האפשרויות, התשובה הנכונה, מה שסומן ״כמעט״, הנושא, ושפת הממשק —
ולא שם, לא מזהה ולא התקדמות. ברק מבצע פעולות באמת: שאלה הבאה,
רמז, הקראה, הדגשת אפשרות, הצגת התמרור, מעבר מסך — ומכריז עליהן
רק אחרי שהצליחו. אין רשת או נגמרה המכסה: המוח המקומי עונה בלי
שגיאה. `privacy.html` עודכן. חמישה קבצים נוספו ל-PRECACHE.
