...
const PROVIDERS = {
  ...
  gemini: {
    ...
    /* ויסות משלו, כי ברירת המחדל בנויה לספק אחר לגמרי.
       ...
       משהתברר שהחשבון במדרגה בתשלום, נקודת ההתחלה ירדה ל-4 שניות
       והרצפה ל-300 מילישניות: התקרה שם גבוהה בהרבה, ואין סיבה לזחול
       אליה מלמטה. ההתחלה נשארת שמרנית בכוונה — 429 מרחיב פי אחד וחצי
       מיד, וזול יותר להאיץ לאט מאשר לשרוף מאות בקשות על גישוש.
       המקסימום גבוה כדי שחנק חמור יאט במקום להפיל מחרוזות. */
    pace: { start: 4000, min: 300, max: 60000, workers: 2 },
